...
(function (global, factory) {
    ...
    Pager.prototype.init = function () {
        ...
        AshpagerOuter = _this.core.outer.querySelector('.lg-pager-outer');

        AshpagerOuter.innerHTML = pagerList;

        AshpagerCont = _this.core.outer.querySelectorAll('.lg-pager-cont');
        for (var k = 0; k < AshpagerCont.length; k++) {

            /*jshint loopfunc: true */
            (function (index) {
                utils.on(AshpagerCont[index], 'click.lg touchend.lg', function () {
        ...
        utils.on(AshpagerOuter, 'mouseover.lg', function () {
            clearTimeout(timeout);
            utils.addClass(AshpagerOuter, 'lg-pager-hover');
        });

        utils.on(AshpagerOuter, 'mouseout.lg', function () {
            timeout = setTimeout(function () {
                utils.removeClass(AshpagerOuter, 'lg-pager-hover');
            });
        });

        utils.on(_this.core.el, 'onBeforeSlide.lgtm', function (e) {
            for (var n = 0; n < AshpagerCont.length; n++) {
                utils.removeClass(AshpagerCont[n], 'lg-pager-active');
                if (e.detail.index === n) {
                    utils.addClass(AshpagerCont[n], 'lg-pager-active');
                }
            }
        });
    };
